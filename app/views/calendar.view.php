...
if (isset($_GET["previous"])) {
    $previousMonth = date('m', strtotime('-1 month', strtotime($year . '-' . sprintf('%02d', $month) . '-01')));
    $month = $previousMonth;
    $calendar = generateCalendar($month, $year, $tasks, $user_id);
}

if (isset($_GET["next"])) {
    $nextMonth = date('m', strtotime('+1 month', strtotime($year . '-' . sprintf('%02d', $month) . '-01')));
    $month = $nextMonth;
    $calendar = generateCalendar($month, $year, $tasks, $user_id);
}
?>
...
<div class="-z-40 bg-purple-900 absolute top-0 left-0 bg-gradient-to-b from-gray-900 via-gray-900 to-purple-800 bottom-0 leading-5 h-full w-full overflow-hidden"></div>
<a href="/" class="group transition-all w-22 flex flex-nowrap text-white hover:no-underline duration-300 rounded-full px-4 py-1 absolute top-7 left-10 border-2 border-red-500 hover:bg-red-500">Back
    <svg class="stroke-white stroke-2 transition-all duration-300 relative -right-1 top-1 w-5 h-5 " viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg" fill="none">
...
<div class="flex flex-col items-center h-screen w-full">
    ...
    <div class="flex flex-row gap-5">
        <form method="get" action="">
            ...
            <button type="submit" class="bg-purple-700 cursor-pointer text-center text-white w-28 font-extrabold flex flex-row items-center px-3 py-3 rounded-xl hover:bg-purple-400 transition-all text-sm">
                ...
                <?= date('Y-m', strtotime('-1 month', strtotime($year . '-' . sprintf('%02d', $month) . '-01'))) ?>
            </button>
        </form>
        <form method="get" action="">
